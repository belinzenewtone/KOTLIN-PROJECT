...
class LocalAIEngine
    ...
    constructor(
        ...
        suspend fun processQuery(query: String): String {
            ...
            return when {
                // Spending queries
                lower.containsAny("spend", "spent", "expense", "cost") -> handleSpendingQuery(lower, userId)

                // Category queries
                lower.containsAny("category", "breakdown", "where does my money") -> handleCategoryQuery(userId)
                ...
                // Task queries
                lower.containsAny("task", "todo", "pending", "to do", "to-do") -> handleTaskQuery(lower, userId)
                ...
                // Summary/overview
                lower.containsAny("summary", "overview", "how am i doing", "report") -> handleSummaryQuery(userId)
        ...
        private suspend fun handleSummaryQuery(userId: String): String {
            val todaySpend =
                transactionDao.getTotalSpendingBetween(
                    DateUtils.todayStartMillis(), DateUtils.todayEndMillis(), userId,
                ).first() ?: 0.0
            ...
            val pendingTasks = taskDao.getPendingTasks(userId).first().size
