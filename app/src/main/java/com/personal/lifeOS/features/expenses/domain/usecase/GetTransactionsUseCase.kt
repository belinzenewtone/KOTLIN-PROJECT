...
class GetTransactionsUseCase @Inject constructor(
    ...
    operator fun invoke(filter: TransactionFilter = TransactionFilter.ALL): Flow<List<Transaction>> {
        return when (filter) {
            ...
            TransactionFilter.TODAY -> repository.getTransactionsBetween(
                DateUtils.todayStartMillis(),
                DateUtils.todayEndMillis()
            )
            TransactionFilter.THIS_WEEK -> repository.getTransactionsBetween(
                DateUtils.weekStartMillis(),
                DateUtils.todayEndMillis()
            )
            TransactionFilter.THIS_MONTH -> repository.getTransactionsBetween(
