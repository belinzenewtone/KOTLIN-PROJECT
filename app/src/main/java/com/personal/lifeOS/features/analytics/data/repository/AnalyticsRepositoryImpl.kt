...
class AnalyticsRepositoryImpl
    ...
    constructor(
        ...
        override fun getAnalytics(): Flow<AnalyticsData> {
            ...
            val monthTransactions = transactionDao.getTransactionsBetween(monthStart, monthEnd, userId)
            val categoryBreakdown = transactionDao.getCategoryBreakdown(monthStart, monthEnd, userId)
            val pendingTasks = taskDao.getPendingCount(userId)
            val completedTasks = taskDao.getCompletedCountBetween(monthStart, monthEnd, userId)
            val upcomingEvents = eventDao.getUpcomingEvents(System.currentTimeMillis(), userId, 50)
