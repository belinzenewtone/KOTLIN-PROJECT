...
class DashboardRepositoryImpl @Inject constructor(
    ...
    override fun getDashboardData(): Flow<DashboardData> {
        ...
        val todaySpendingFlow = transactionDao.getTotalSpendingBetween(todayStart, todayEnd)
        val weekSpendingFlow = transactionDao.getTotalSpendingBetween(weekStart, todayEnd)
        val monthSpendingFlow = transactionDao.getTotalSpendingBetween(monthStart, monthEnd)
        val upcomingEventsFlow = eventDao.getUpcomingEvents(now, 5)
        val pendingTasksFlow = taskDao.getPendingCount()
        val completedTodayFlow = taskDao.getCompletedCountBetween(todayStart, todayEnd)
        val recentTxFlow = transactionDao.getTransactionsBetween(weekStart, todayEnd)
        ...
        return combine(
            ...
            DashboardIntermediate(todaySpend, weekSpend, monthSpend, events.map {
            ...
            val weeklyData = buildWeeklySpending(recentTx.map {
            ...
            DashboardData(
                ...
                recentTransactions = recentTx.take(5).map {
                    RecentTransaction(it.id, it.amount, it.merchant, it.category, it.date)
                },
